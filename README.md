...
1. Add a CV and supporting documents under **Build my profile**. One action stores the originals, extracts source-backed facts, creates the internal profile/Master CV/research context, and writes a readable `data/profile_summaries/applicant-profile-pN-tN.md` file. Previously uploaded Vault documents are detected and reused. Transcript and degree files are stored and parsed but supply profile facts only after their existing authenticity check is completed in Advanced tools.
...
4. Use **Find relevant supervisors** for an application. Official faculty-page leads show extracted interests, listed work, and source dates while clearly marked **Needs review**. Reviewed profiles show verified recent publications, demonstrated applicant overlap, proposed directions, Research Fit, supervision and contact policy separately. **Pursue** shortlists a professor for that application; **Reject** hides the recommendation but keeps it restorable, including after another search. **Research deeper** checks the official profile and linked same-host research pages in a stoppable, resumable operation. New research remains reviewable and does not prove supervision availability or authorize outreach.
