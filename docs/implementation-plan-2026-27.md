...
Expose a small `DocumentStorage` interface (put, get, existence, hash verification, export) so document IDs and application records do not depend on local paths. Implement `LocalDocumentStorage` in Slice 1. An `S3CompatibleDocumentStorage` backend, including a Railway Storage Bucket, is optional after the local Vault and package builder work. [Railway buckets are private and support presigned access](https://docs.railway.com/storage-buckets), but Railway currently lists no bucket-side server encryption, object versioning, or object locks; the application must retain its own version/backup controls. Highly sensitive files require explicit opt-in and application-side encryption before any cloud upload.

A hosted Railway deployment would need access control for the Streamlit app, authenticated downloads, secret management, backups, and a persistent database volume. [Railway volumes persist service data but do not support replicas and can cause brief deployment downtime](https://docs.railway.com/volumes/reference). A file designated local-only is unavailable to a hosted instance: build any package requiring it locally, or explicitly change the storage policy. No cloud deployment is required for the local applicant workflow.
